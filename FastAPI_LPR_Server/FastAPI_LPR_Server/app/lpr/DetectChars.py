...
# constants for checkIfPossibleChar, this checks one possible char only (not comparing to another chars)
MIN_PIXEL_WIDTH = 2
MIN_PIXEL_HEIGHT = 8
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
    ...
    for possibleChar in listOfPossibleChars:                        # for each possible char in the one big list of chars
        ...
        if len(listOfMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS:  # if current possible list of matching chars is not long 
            continue      # enough to constitute a possible plate jump back to the top of the for loop and try again with next char
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    for possibleMatchingChar in listOfChars:                # for each char in the big list
